...
namespace QuantConnect.Lean.Engine.DataFeeds.Enumerators.Factories
{
    ...
    public class FineFundamentalSubscriptionEnumeratorFactory : ISubscriptionEnumeratorFactory
    {
        ...
        private static readonly ConcurrentDictionary<int, List<string>> FineFilesCache
        ...
        private SubscriptionDataSource GetSource(FineFundamental fine, SubscriptionDataConfig config, DateTime date)
        {
            ...
            if (!File.Exists(source.Source))
            {
                ...
                try
                {
                    directoryName = Path.GetDirectoryName(source.Source);
                }
                catch
                {
                    directoryName = null;
                ...
                if (!FineFilesCache.TryGetValue(cacheKey, out availableFiles))
                {
                    try
                    {
                        availableFiles = Directory.GetFiles(directoryName,"*.zip")
                            .OrderBy(x => x).ToList();
                    }
                    catch
                    {
                        ...
                        if (!_isLiveMode)
                        {
                            // only cache if not live
                            FineFilesCache[cacheKey] = new List<string>();
                        }
                        return source;
                    }
                    ...
                    {
                        // only cache if not live
                        FineFilesCache[cacheKey] = availableFiles;
                    }
                }
                ...
                if (current <= date)
                {
                    // we found the date, return the right source
